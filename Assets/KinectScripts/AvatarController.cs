      ...
      //  if (!isMoveX)
       ...
       // }
        ...
        Vector3 targetPos = Kinect2AvatarPos(trans, verticalMovement);
           ...
           if (!isMoveZ)
           {
               targetPos.z = bodyRoot.localPosition.z;
           }
          if (!isMoveX)
         {
            targetPos.x = bodyRoot.localPosition.x;
             }

        if (limX > 0)
        {
            targetPos.x = Mathf.Clamp(targetPos.x,-limX,limX);
        }
        if (limZ > 0)
        {
            targetPos.z = Mathf.Clamp(targetPos.z, -limZ, limZ);
        }
