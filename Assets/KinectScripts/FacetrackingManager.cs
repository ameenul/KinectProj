...
public class FacetrackingManager : MonoBehaviour 
{
	...
	public Texture2D usersClrTex,baru;
			...
			if(rc >= 0)
			{
				...
				if(computeColorMap)
				{
					if(FacetrackingWrapper.PollVideo(ref videoBuffer, ref colorImage))
					{
                        ...
                        //
                      //  baru = new Texture2D(faceRect.width, faceRect.height, TextureFormat.ARGB32, false);
                      //  baru.SetPixels(usersClrTex.GetPixels(Mathf.FloorToInt(faceRect.x), Mathf.FloorToInt(faceRect.y), Mathf.FloorToInt(faceRect.width), Mathf.FloorToInt(faceRect.height)));
                        //
	...
	void OnGUI()
	{
		if(facetrackingInitialized)
		{
			if(computeColorMap && displayColorMap)
			{
               ...
               // GUI.DrawTexture(new Rect(usersClrTex.width, 0, -usersClrTex.width, usersClrTex.height), usersClrTex);
...
//				}
	...
	private void DrawFacetrackerRect(Texture2D aTexture, bool bApplyTexture)
	{
        ...
        if (baru == null) {
            ...
            // baru = new Texture2D(faceRect.width, faceRect.height);
           // baru = new Texture2D(100, 100);
        }
	...
	////	DrawLine(aTexture, pt1, pt2, color);
       ...
       baru = new Texture2D(faceRect.width, faceRect.height);
        ...
        int poinX = (aTexture.width) - faceRect.x - faceRect.width + wi;
        int poinY = (aTexture.height) - faceRect.y - faceRect.height + he;

        if(poinX>=0 && poinY >= 0)
        { 
        Color[] faceCol = aTexture.GetPixels(poinX, poinY, faceRect.width, faceRect.height);
        baru.SetPixels(faceCol);
        baru.Apply();
        // Texture2D baruFlip = FlipTexture(baru,false);
        // myCube.GetComponent<Renderer>().material.mainTexture = baruFlip;
        myCube.GetComponent<Renderer>().material.mainTexture = baru;
        myCube.GetComponent<Renderer>().material.SetTextureScale("_MainTex", new Vector2(-1, 1));
        /////////////////
         }
    }
